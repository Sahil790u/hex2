...
def start_message(message):
    if message.chat.id in approved_users:
        start_text = (
            ...
            "<b>/generate</b> - Select payload type (Multi or Hex) and generate file.\n"
            "<b>/generatemulti &lt;number&gt;</b> - Generate file with Multi payload.\n"
            "<b>/generatehex &lt;number&gt; &lt;payload_value&gt;</b> - Generate file with Hex payload.\n"
...
# ------------------------------------------------------------------------------
# /generate Command – Interactive conversation to choose payload type.
...
def generate_command(message):
    ...
    prompt = (
        ...
        "• <b>Multi</b>: Generates payload like: xb0\\x45\\x5c\\x74\\x9d\\xb4\\xc8\\xd9\n"
        "• <b>Hex</b>: Generates payload like: \"\\x91\", \"\\x91\", \"\\x91\"\n"
...
def process_payload_choice(message):
    chosen_type = message.text.strip().lower()  # 'multi' or 'hex'
    sahil_bot.send_message(
        message.chat.id,
...
# ------------------------------------------------------------------------------
# /generatemulti Command – Directly generate Multi payload.
@sahil_bot.message_handler(commands=['generatemulti'])
def generate_multi(message):
    if message.chat.id not in approved_users:
        sahil_bot.send_message(
            message.chat.id,
            "⛔ <b>Aap is bot ko use nahi kar sakte. Admin approval required.</b> 🚫",
    ...
    try:
        ...
        if len(parts) < 2:
            sahil_bot.send_message(
                message.chat.id,
                "❌ <b>Invalid format!</b> Use: /generatemulti &lt;number&gt;",
                parse_mode="HTML"
            )
            return
        count = int(parts[1])
        if count <= 0:
            sahil_bot.send_message(
                message.chat.id,
                "❌ <b>Number must be greater than 0!</b>",
                parse_mode="HTML"
            )
            return
        file_path = generate_and_save_payload("multi", count, payload_value="xb0\\x45\\x5c\\x74\\x9d\\xb4\\xc8\\xd9")
        if file_path:
            ...
            sahil_bot.send_message(
                message.chat.id,
                "❌ <b>Error: Could not generate Multi payload.</b> ⚠️",
                parse_mode="HTML"
            )
    except Exception:
        sahil_bot.send_message(
...
# ------------------------------------------------------------------------------
# /generatehex Command – Directly generate Hex payload.
@sahil_bot.message_handler(commands=['generatehex'])
def generate_hex(message):
    if message.chat.id not in approved_users:
        sahil_bot.send_message(
            message.chat.id,
            "⛔ <b>Aap is bot ko use nahi kar sakte. Admin approval required.</b> 🚫",
    ...
    try:
        ...
        if len(parts) < 3:
            sahil_bot.send_message(
                message.chat.id,
                "❌ <b>Invalid format!</b> Use: /generatehex &lt;number&gt; &lt;payload_value&gt;",
                parse_mode="HTML"
            )
            return
        count = int(parts[1])
        payload_value = parts[2].strip()
        if not (payload_value.startswith("\\x") and len(payload_value) == 4):
            sahil_bot.send_message(
                message.chat.id,
                "❌ <b>Invalid payload value!</b> Use a value like \\x91 or \\x51.",
                parse_mode="HTML"
            )
            return
        file_path = generate_and_save_payload("hex", count, payload_value)
        if file_path:
            ...
            sahil_bot.send_message(
                message.chat.id,
                "❌ <b>Error: Could not generate Hex payload.</b> ⚠️",
                parse_mode="HTML"
            )
    except Exception:
        sahil_bot.send_message(
